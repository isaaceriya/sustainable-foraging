...
public class ForageService {
    ...
    private void checkDuplicate(Forager forager, Result<Forager> result){
        if(foragerRepository.findAll().stream().anyMatch(i -> Boolean.parseBoolean(i.getLastName()))) {
            result.addErrorMessage(String.format("Item '%s' is a duplicate.", forager.getLastName()));
        }
    }
